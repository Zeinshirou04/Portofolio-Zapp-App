...
function MetaItem({
  ...
  return (
    <div className="flex items-start gap-3">
      <FontAwesomeIcon
        icon={icon}
        className="h-4 w-4 text-volt mt-0.5 shrink-0"
      />
      <div>
        <p className="font-sans text-xs text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-0.5">
          {label}
        </p>
        <p className="font-sans text-sm text-gray-800 dark:text-gray-200 font-medium">
          {value}
        </p>
      </div>
    </div>
      ...
      <ol className="relative border-l border-gray-200 dark:border-zinc-700 space-y-8 pl-6">
        {timelines.map((t) => (
          <li key={t.id} className="relative">
            ...
            <time className="font-sans text-xs text-gray-400 dark:text-gray-500">
              {formatDate(t.occurred_at)}
            </time>
            <h3 className="font-display font-bold text-base text-gray-900 dark:text-gray-100 mt-0.5 mb-1">
              {t.title}
            </h3>
            {t.description && (
    ...
    <div>
      ...
      <ul className="space-y-3">
        {contributors.map((c) => (
          <li
            key={c.id}
            className="flex items-center gap-3 bg-gray-50 dark:bg-zinc-800/60
          ...
          >
            <div className="h-9 w-9 rounded-full bg-volt/15 flex items-center justify-center shrink-0">
              <span className="font-display font-bold text-sm text-volt">
                {c.name.charAt(0).toUpperCase()}
              </span>
            </div>
            <div>
              <p className="font-sans text-sm font-semibold text-gray-800 dark:text-gray-200">
                {c.name}
              </p>
...
function ScreenshotGallery({ images }: { images: Project["images"] }) {
        ...
        {certs.map((img) => (
          <motion.div
            ...
            className="relative overflow-hidden rounded-xl
                       border border-gray-100 dark:border-zinc-800
                       bg-gray-50 dark:bg-zinc-900 aspect-3/4"
          >
            <Image
              src={img.path}
              alt={img.caption ?? "Certificate"}
              fill
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 lg:gap-16">
        ...
        <div className="lg:col-span-2 space-y-12">
          ...
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <CertificateGallery images={project.images} />
          </motion.div>
